...
//Postprocessing usefull vars
	...
	GLint			u_FBTexSizeX;
	GLint			u_FBTexSizeY;
	GLint			u_ScreenToNextPixelX;
	GLint			u_ScreenToNextPixelY;
	GLint			u_zFar;
	GLint			u_zNear;
	GLint			u_rotationBlurPosition;
	GLint			u_rotationBlurPositionOld;
...
extern cvar_t	*r_postprocess_multipart;
...
glsl_light_t	glsl_light[35];
int 		glsl_lights;
//test
int glsl_lightmapIndex; 
...
typedef struct shaderCommands_s 
{
	...
	vec3_t		_normal[SHADER_MAX_VERTEXES] QALIGN(16);
	vec3_t		tangent[SHADER_MAX_VERTEXES] QALIGN(16);
	int		tangents[SHADER_MAX_VERTEXES] QALIGN(16);
	vec3_t		binormal[SHADER_MAX_VERTEXES] QALIGN(16);
...
static ID_INLINE void R_GLSL_SetUniform_u_NumLights(glslProgram_t *program, const int value) {
	if (program->u_NumLights == value)
		return;

	program->u_NumLights = value;
	qglUniform1iARB(program->u_NumLights, value);
}
...
static ID_INLINE void R_GLSL_SetUniform_LightmapControl(glslProgram_t *program, int value) {
	if (program->u_thisStageLightmap == value)
		return;

	qglUniform1iARB(program->u_thisStageLightmap, value);
}

static ID_INLINE void R_GLSL_SetUniform_u_etype(glslProgram_t *program, int value) {
	if (program->u_etype == value)
		return;

	qglUniform1iARB(program->u_etype, value);
}
...
static ID_INLINE void R_GLSL_SetUniform_u_zNear(glslProgram_t *program, GLfloat value) {
...
static ID_INLINE void R_GLSL_SetUniform_u_rotationBlurPosition(glslProgram_t *program, const vec3_t value) {
	qglUniform3fARB(program->u_rotationBlurPosition, value[0], value[1], value[2]);
}

static ID_INLINE void R_GLSL_SetUniform_u_rotationBlurPositionOld(glslProgram_t *program, const vec3_t value) {
	qglUniform3fARB(program->u_rotationBlurPositionOld, value[0], value[1], value[2]);
}
