...
static glslProgram_t *R_GLSL_AllocProgram(void) {
	...
	program->u_FBTexSizeX					= -1;
	program->u_FBTexSizeY					= -1;
	program->u_ScreenToNextPixelX			= -1;
	program->u_ScreenToNextPixelY			= -1;
	program->u_zFar							= -1;
	program->u_zNear						= -1;
    program->u_rotationBlurPosition         = -1;
    program->u_rotationBlurPositionOld      = -1;
	program->u_NumLights					= -1;
	program->u_SunPos                       = -1;
	...
	while (token[0]) {
		if (!Q_stricmp(token, "uniform")) {
			...
			if (!Q_stricmp(token, "int")) {
				...
				} else if (!Q_stricmp(token, "u_thisStageLightmap;")) {
					program->u_thisStageLightmap = qglGetUniformLocationARB(program->program, "u_thisStageLightmap");
				} else if (!Q_stricmp(token, "u_etype;")) {
					program->u_etype = qglGetUniformLocationARB(program->program, "u_etype");
				...
				} else if (!Q_stricmp(token, "u_NumLights;")) {
					program->u_NumLights = qglGetUniformLocationARB(program->program, "u_NumLights");
				} else {
				...
				}  else if (!Q_stricmp(token, "u_zNear;")) {
					program->u_zFar = qglGetUniformLocationARB(program->program, "u_zNear");
			...
			} else if (!Q_stricmp(token, "mat3")) {
				token = COM_ParseExt(text, qfalse);
				if (!Q_stricmp(token, "u_Lightinfo[32];")) {
					program->u_Lightinfo = qglGetUniformLocationARB(program->program, "u_Lightinfo");
				} else {
					ri.Printf(PRINT_WARNING, "WARNING: uniform mat3 %s unrecognized in program %s\n", token, program->name);
				}
				...
				} else if (!Q_stricmp(token, "u_SunPos;")) {
					program->u_SunPos = qglGetUniformLocationARB(program->program, "u_SunPos");
				} else if (!Q_stricmp(token, "u_rotationBlurPosition;")) {
					program->u_rotationBlurPosition = qglGetUniformLocationARB(program->program, "u_rotationBlurPosition");
				} else if (!Q_stricmp(token, "u_rotationBlurPositionOld;")) {
					program->u_rotationBlurPositionOld = qglGetUniformLocationARB(program->program, "u_rotationBlurPositionOld");
				} else {
...
static qboolean R_GLSL_LoadProgram(glslProgram_t *program, const char *name, const char *programVertexObjects, int numVertexObjects, const char *programFragmentObjects, int numFragmentObjects) {
	...
	for (i = 0, str = (char *)programVertexObjects; i < numVertexObjects; i++, str += MAX_QPATH) {
		...
		if (!status) {
			...
			/* exit */
			ri.Printf( PRINT_WARNING,  "Couldn't compile vertex shader for program %s\n", name);
			return qfalse;
		}
	...
	size2=0;
	size3=0;
	for (i=0;i<20000;i++) buffer_fp_final[i]=0;
	/* fragment program */
	for (i = 0, str = (char *)programFragmentObjects; i < numFragmentObjects; i++, str += MAX_QPATH) {
		ri.Printf(PRINT_DEVELOPER, "^4... loading '%s'\n", str);

		size3 = ri.FS_ReadFile(str, (void **)&buffer_fp[i]);
		size+=size3;
		temp=(char *)buffer_fp[i];
		sprintf(&buffer_fp_final[size2],"%s",temp);
		size2+=size3;
		buffer_fp_final[size2]=0;
		if (!buffer_fp[i]) {
			ri.Printf( PRINT_WARNING,  "^6Couldn't load %s\n", str);
			return qfalse;
		}
		if (i>19999) {
			ri.Printf( PRINT_WARNING,  "^6Combined GLSL program too long: %s\n", str);
			return qfalse;
		}

	}
	buffer_fp_temp=&buffer_fp_final;
 //ri.Printf(PRINT_ALL,"---------------------------------------------");
 //ri.Printf(PRINT_ALL,"%s",buffer_fp_temp);
		/* compile fragment shader */
		shader_fp = qglCreateShaderObjectARB(GL_FRAGMENT_SHADER_ARB);
		qglShaderSourceARB(shader_fp, 1, &buffer_fp_temp, NULL);
		...
		if (!status) {
			...
			/* exit */
			ri.Printf( PRINT_WARNING,  "Couldn't compile fragment shader for program %s\n", name);
			return qfalse;
		}
    ...
    qglBindAttribLocationARB(program->program, 6, "u_Tangent");
    qglBindAttribLocationARB(program->program, 7, "u_Binormal");
    qglBindAttribLocationARB(program->program, 14, "u_Normal");
	...
	if (!status) {
		...
		/* exit */
		ri.Printf( PRINT_WARNING,  "Couldn't link shaders for program %s\n", name);
		return qfalse;
	}
