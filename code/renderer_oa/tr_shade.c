...
void myMatrix( const float *modelMatrix, const float *projectionMatrix) {
	int i;

	for ( i = 0 ; i < 16 ; i++ ) {
     mymat[i] = modelMatrix[ i ] * projectionMatrix[ i ];
	}
...
==========================
*/
void my_GlMultMatrix( const float *a, const float *b, float *out ) {
	int		i, j;
...
void xR_RotateForViewer (void)
{
	...
	// convert from our coordinate system (looking down X)
	// to OpenGL's coordinate system (looking down -Z)
	my_GlMultMatrix( viewerMatrix, xs_flipMatrix, tr.or.modelMatrix );
...
void GLSL_DefaultProgram_Feeder(shaderStage_t *pStage, shaderCommands_t *input)
{
 ...
 int stage, bundle;
	...
	{
		if (pStage->bundle[0].tcGen==2)
            {
            R_GLSL_SetUniform_TCGen0(program, 3);
            }
		else
